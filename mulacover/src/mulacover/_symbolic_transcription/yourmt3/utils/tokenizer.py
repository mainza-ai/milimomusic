...
from mulacover._symbolic_transcription.yourmt3.utils.event_codec import (
    FastCodec as Codec,
)
from mulacover._symbolic_transcription.yourmt3.utils.note_event_dataclasses import (
    NoteEvent,
)
from mulacover._symbolic_transcription.yourmt3.utils.note2event import note_event2event
from mulacover._symbolic_transcription.yourmt3.utils.event2note import (
    event2note_event,
    note_event2note,
)
...
class EventTokenizerBase(ABC):
    ...
    def __init__(
        ...
        if isinstance(base_codec, str):
            ...
            # Initialize codec
            self.codec = Codec(
                special_tokens=special_tokens + extra_tokens,
...
class NoteEventTokenizer(EventTokenizerBase):
    ...
    def __init__(
        ...
        self.ids_to_ignore_decoding = [
            self.codec.special_tokens.index(t) for t in ignore_decoding_tokens
        ]
    ...
    def encode_plus(
        ...
        note_events: List[NoteEvent],
        tie_note_events: Optional[List[NoteEvent]] = None,
        start_times: float = 0.0,  # Fixing bug: start_time --> start_times
    ...
    def encode_task(
        self, task_events: List[Event], max_length: Optional[int] = None
    ) -> List[int]:
    ...
    def decode(
        ...
        Tuple[List[NoteEvent], List[NoteEvent], List[Tuple[int]], List[Event], int],
    ]:
        ...
        """
        if self.debug_mode:
            ignored_tokens_from_input = [
                t for t in tokens if t in self.ids_to_ignore_decoding
            ]
        ...
        note_events, tie_note_events, last_activity, err_cnt = event2note_event(
            events, start_time, True, self.tps
        )
        if return_events:
    ...
    def decode_batch(
        ...
        Tuple[
            List[
                Tuple[List[NoteEvent], List[NoteEvent], List[Tuple[int]], List[float]]
            ],
            int,
        ],
        Tuple[
        ...
        for tokens, start_time in zip(batch_tokens, start_times):
            if return_events:
                note_events, tie_note_events, last_activity, events, err_cnt = (
                    self.decode(tokens, start_time, return_events)
                )
                list_events.append(events)
            else:
                note_events, tie_note_events, last_activity, err_cnt = self.decode(
                    tokens, start_time, return_events
                )

            zipped_note_events_and_tie.append(
                (note_events, tie_note_events, last_activity, start_time)
            )
    ...
    def decode_list_batches(
        ...
        Tuple[
            List[
                List[
                    Tuple[
                        List[NoteEvent], List[NoteEvent], List[Tuple[int]], List[float]
                    ]
                ]
            ],
            Counter[str],
