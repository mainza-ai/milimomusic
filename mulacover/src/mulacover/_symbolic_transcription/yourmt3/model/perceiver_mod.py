...
from mulacover._symbolic_transcription.yourmt3.model.perceiver_helper import (
    MoEModelOutputWithCrossAttentions,
)
from mulacover._symbolic_transcription.yourmt3.model.perceiver_helper import (
    PerceiverTFPreTrainedModel,
    PerceiverTFConfig,
)
from mulacover._symbolic_transcription.yourmt3.model.positional_encoding import (
    AlibiPositionalBias,
    get_rotary_emb,
)
from mulacover._symbolic_transcription.yourmt3.model.ops import get_layer_norm
from mulacover._symbolic_transcription.yourmt3.model.ff_layer import get_ff_layer
...
class PerceiverTFTrainablePE(nn.Module):
    ...
    def __init__(
        self,
        position_encoding_type: Literal["trainable", "tkd", "td", "tk", "kdt"],
        max_t: int,
        k: int,
        d: int,
    ) -> None:
...
class PerceiverAlibiSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        # Layer normalization
        self.layernorm1 = get_layer_norm(
            q_dim, config.layer_norm_type, config.layer_norm_eps
        )
        if is_cross_attention:
            self.layernorm2 = get_layer_norm(
                kv_dim, config.layer_norm_type, config.layer_norm_eps
            )
        else:
        ...
        if config.position_encoding_type == "alibi":
            self.alibi_bias = AlibiPositionalBias(
                heads=num_heads, total_heads=num_heads, trainable_slope=False
            )
        elif config.position_encoding_type == "alibit":
            self.alibi_bias = AlibiPositionalBias(
                heads=num_heads, total_heads=num_heads, trainable_slope=True
            )
        else:
    ...
    def forward(
        ...
        if self.alibi_bias is not None:
            ...
            attention_scores += self.alibi_bias(
                q_seq_len, k_seq_len
            )  # auto-broadcasting to (b, num_heads, q_seq_len, k_seq_len)
...
class PerceiverAlibiAttention(nn.Module):
    ...
    def __init__(
        ...
        if is_cross_attention and qk_channels is None:
            ...
            if v_channels is None:
                v_channels = qk_channels
        self.self = PerceiverAlibiSelfAttention(
            config,
...
class PerceiverAlibiLayer(nn.Module):
    ...
    def __init__(
        ...
        self.seq_len_dim = 1
        self.attention = PerceiverAlibiAttention(
            config,
            is_cross_attention=is_cross_attention,
            qk_channels=qk_channels,
            v_channels=v_channels,
        ...
        self.layernorm = get_layer_norm(
            q_dim, config.layer_norm_type, config.layer_norm_eps
        )
        # self.layernorm = nn.LayerNorm(q_dim)
        self.mlp = get_ff_layer(
    ...
    def forward(
        ...
        """
        layer_output, router_logits = apply_chunking_to_forward(
        ...
        outputs = (
            (layer_output,) + outputs + (router_logits,)
        )  # add router_logits to outputs
        return outputs

    def feed_forward_chunk(self, attention_output):
        layer_output = self.layernorm(attention_output)
        layer_output, router_logits = self.mlp(
            layer_output
        )  # router_logits is returned only when using MoE.
...
class PerceiverTFEncoderBlock(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.local_transformer):
            ...
            layer_head_mask = (
                local_head_mask[i] if local_head_mask is not None else None
            )
        ...
        for i, layer_module in enumerate(self.temporal_transformer):
            ...
            layer_head_mask = (
                temporal_head_mask[i] if temporal_head_mask is not None else None
            )

            if i == 0 and pos_emb_temporal is not None:
                # Add temporal positional embeddings to the hidden_states.
                hidden_states = (
                    hidden_states + pos_emb_temporal[:t]
                )  # pos_emb_temporal: (T, D)
        ...
        last_hideen_state = rearrange(
            last_hideen_state, "(b k) t d -> b t k d", b=batch_size
        )
...
class PerceiverTFEncoder(PerceiverTFPreTrainedModel):
    ...
    def __init__(
        ...
        if sca_use_query_residual is None:
            self.sca_use_query_residual = (
                config.sca_use_query_residual
            )  # True by default
        ...
        if self.position_encoding_type == "rope":
            # (Modified) RoPE
            self.rotary_emb_sca = get_rotary_emb(
                config.num_cross_attention_heads,
                config.rope_type_sca,
                config.rope_partial_pe,
                config.rope_trainable,
            )
            self.rotary_emb_latent = get_rotary_emb(
                config.num_cross_attention_heads,
                config.rope_type_latent,
                config.rope_partial_pe,
                config.rope_trainable,
            )
            self.rotary_emb_temporal = get_rotary_emb(
                config.num_cross_attention_heads,
                config.rope_type_temporal,
                config.rope_partial_pe,
                config.rope_trainable,
            )
        else:
        ...
        if self.position_encoding_type in ["alibi", "alibit", "rope", None]:
            ...
            max_t = (
                int(config.num_max_positions) + 10
            )  # 10 is headroom for future task tokens...
            self.pos_emb = PerceiverTFTrainablePE(
                self.position_encoding_type, max_t, k, d
            )
            """
    ...
    def _forward(
        ...
        inputs_mask: Optional[
            torch.FloatTensor
        ] = None,  # (B, F) Mask freq. of inputs in SCA.
        ...
        local_head_mask = self.get_head_mask(
            local_head_mask,
            self.config.num_blocks * self.config.num_local_transformers_per_block,
        )
        temporal_head_mask = self.get_head_mask(
            temporal_head_mask,
            self.config.num_blocks * self.config.num_temporal_transformers_per_block,
        )

        # Prepare attention mask: not implemented

        # Expand the latent embeddings by t: (B, K, D) --> (B, T, K, D)
        latent_embeddings = self.latent_array(
            batch_size=batch_size
        )  # (B, num_latents, d_latents)
        expanded_latent_embeddings = latent_embeddings.unsqueeze(1).expand(
            -1, t, -1, -1
        )
