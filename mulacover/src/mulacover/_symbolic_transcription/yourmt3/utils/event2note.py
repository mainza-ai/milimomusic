...
DECODING_ERR_TYPES = [
    "decoding_time",
    "Err/Missing prg in tie",
    "Err/Missing tie",
    "Err/Shift out of range",
    "Err/Missing prg",
    "Err/Missing vel",
    "Err/Multi-tie type 1",
    "Err/Multi-tie type 2",
    "Err/Unknown event",
    "Err/onset not found",
    "Err/active ne incomplete",
    "Err/merging segment tie",
    "Err/long note > 10s",
]


def event2note_event(
    events: List[Event], start_time: float = 0.0, sort: bool = True, tps: int = 100
) -> Tuple[List[NoteEvent], List[NoteEvent], List[Tuple[int]], Counter[str]]:
    ...
    last_activity = (
        []
    )  # For activity check and last activity of segment. [(program, pitch), ...]
    ...
    for i, e in enumerate(events):
        try:
            ...
            error_type = str(ve)
            error_counter[error_type] = error_counter.get(error_type, 0.0) + 1
...
def note_event2note(
    ...
    if sort:
        note_events.sort(
            key=lambda ne: (ne.time, ne.is_drum, ne.pitch, ne.velocity, ne.program)
        )
...
def merge_zipped_note_events_and_ties_to_notes(
    zipped_note_events_and_ties, force_note_off_missing_tie=True, fix_offset=True
) -> Tuple[List[Note], Counter[str]]:
    ...
    for nes, tie_nes, last_activity, start_time in zipped_note_events_and_ties:
        if prev_last_activity is not None and force_note_off_missing_tie:
            ...
            for (
                prog_pitch_pla
            ) in prev_last_activity:  # (program, pitch) of previous last active notes
    ...
    notes, err_cnt = note_event2note(
        merged_note_events, tie_note_events=None, fix_offset=fix_offset
    )
