...
async def test_minimax_strict_inference_raises_when_real_inference_fails(monkeypatch):
    ...
    with patch("app.providers.minimax_provider._MLX_AUDIO_AVAILABLE", True), \
         patch("os.path.isdir", return_value=True), \
         patch("app.providers.minimax_provider.run_real_minimax_inference", side_effect=RuntimeError("VRAM Allocation Failed")):
