...
from mulacover._symbolic_transcription.yourmt3.model.ops import count_parameters
...
class GroupLinearFlatten(nn.Module):
    ...
    def __init__(self, in_features, flatten_out_features, num_groups, use_bmm=True):
        ...
        self.weight = nn.Parameter(
            torch.Tensor(num_groups, self.out_features_per_group, in_features)
        )
    ...
    def forward(self, input):
        ...
        if self.use_bmm:
            ...
            output = rearrange(
                output_bmm, "k d_out (b t) -> b t (k d_out)", b=batch_size, t=t, k=k
            )
        else:
...
class MultiChannelLinearProjection(nn.Module):
    ...
    def __init__(self, in_ch=26, in_dim=128, out_ch=13, out_dim=512):
        ...
        if in_dim * self.in_ch_per_group == out_dim:
            ...
            self.linear = nn.Linear(
                in_features=self.linear_in_dim, out_features=out_dim, bias=False
            )
    ...
    def forward(self, x):
        ...
        x = rearrange(
            x,
            "b t (c1 c2) d -> b c1 t (c2 d)",
            c1=self.linear_in_ch,
            c2=self.in_ch_per_group,
        )
        return self.linear(x)


def get_multi_channel_projection_layer(
    input_shape: Tuple[int], output_shape: Tuple[int], proj_type: str
) -> nn.Module:
...
class FlattenMLP(nn.Module):
    ...
    def __init__(
        ...
        assert (
            flatten_out_features % in_features == 0
        ), "flatten_out_features should be divisible by in_features."

        # Define layers
        self.layers = nn.Sequential(
            nn.Flatten(2, 3),
            nn.Linear(self.flat_in_dim, hidden_dim),
...
class LinearProjection(nn.Module):
    ...
    def __init__(self, in_features, flatten_out_features, num_groups):
        ...
        self.projection_layer = nn.Linear(
            in_features=self.flat_in_dim, out_features=flatten_out_features, bias=False
        )
    ...
    def forward(self, x):
        ...
        x_flattened = x.reshape(
            batch_size, t, -1
        )  # Flattening num_groups and in_features
...
class DepthwiseConvProjection(nn.Module):
    ...
    def __init__(self, in_features, flatten_out_features, num_groups, depth):
        ...
        self.fc = nn.Linear(
            num_groups * d_out * (in_features - depth + 1), flatten_out_features
        )
...
def get_projection_layer(
    ...
    if "linear" == proj_type:
        return LinearProjection(
            in_features=d_source, flatten_out_features=d_target, num_groups=k_source
        )
    elif "mlp" in proj_type:
        if "gelu" in proj_type:
            return FlattenMLP(
                in_features=d_source,
        ...
        if "conv4" == proj_type:
            return DepthwiseConvProjection(
                in_features=d_source,
                flatten_out_features=d_target,
                num_groups=k_source,
                depth=4,
        ...
        assert (
            d_source % k_source == 0
        ), "d_source and k_source must be divisible for group_linear projection."
        return GroupLinearFlatten(
            in_features=d_source,
            flatten_out_features=d_target,
            num_groups=k_source,
            use_bmm=True,
