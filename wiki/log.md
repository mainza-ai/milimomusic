...
## [2026-09-16] fix | Track Extension Root Cause Fixes: Universal Audio Resolution, CORS Fortification & Full Pipeline Recovery
Conducted exhaustive production audit following track extension failures and CORS 500 reports:
1. Root Cause 1 (CWD-dependent File Resolution): When the backend process runs with CWD `backend/`, relative path strings (`generated_audio/...`) resolved to `backend/generated_audio/` instead of `<REPO_ROOT>/generated_audio/` where `get_generated_audio_dir()` writes files. `_resolve_audio_file()` in `karaoke.py` only checked paths relative to CWD and missed `<REPO_ROOT>/generated_audio/`. Fixed by referencing `get_repo_root()`, `get_generated_audio_dir()`, and `get_data_dir()`, and testing alternate common audio extensions (`.wav`, `.mp3`, `.flac`, `.ogg`).
2. Root Cause 2 (Audio Concatenation & Formats): `_read_wav_float` in `minimax_provider.py` now automatically resolves paths via `_resolve_audio_file()`, and supports multi-codec decoding (SoundFile, Scipy, Wave, TorchAudio). Added sample rate matching and resampling in `concatenate_and_crossfade_audio()` if parent and extension rates differ.
3. Root Cause 3 (Intermediate Files & Output Naming): Standardized output master path to `/audio/{job_id}.wav` (mirrored to `song_{job_id}.wav` and backend directory), and cleans up intermediate extension segments (`*_ext_segment.wav`).
4. Root Cause 4 (CORS & Exception Envelope): Added `allow_origin_regex=r"^https?://(localhost|127\.0\.0\.1)(:[0-9]+)?$"` and `expose_headers=["*"]` to FastAPI CORSMiddleware. Updated both `unhandled_exception_handler` and `validation_exception_handler` with dynamic `_build_cors_headers(origin)` to prevent unhandled 500s or 422s from masking as CORS network errors.
5. Root Cause 5 (Dual Stems & Muscriptor Path Decoupling): Updated `pipeline.py`, `real_separator.py`, and `muscriptor_provider.py` to write and read exclusively via `get_generated_audio_dir()` and mirror outputs into `backend/generated_audio`.
