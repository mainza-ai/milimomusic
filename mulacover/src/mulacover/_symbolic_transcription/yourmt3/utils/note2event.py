...
from mulacover._symbolic_transcription.yourmt3.utils.note_event_dataclasses import Event
...
def validate_notes(
    notes: Tuple[List[Note]], minimum_offset: Optional[bool] = 0.01, fix: bool = True
) -> List[Note]:
        ...
        for note in list(notes):
            if note.onset == None:
                ...
                if fix:
                    ...
                    print(
                        f"✅\033[92m Fixed! Setting offset to onset + {MINIMUM_OFFSET_TIME}.\033[0m"
                    )
...
def extract_notes_selected_by_programs(
    notes: List[Note], programs: Set[int], sort: bool = True
) -> List[Note]:
...
def note2note_event(
    notes: List[Note], sort: bool = True, return_activity: bool = True
) -> List[NoteEvent]:
    """
    note2note_event:
    ...
    for note in notes:
        ...
        note_events.append(
            NoteEvent(note.is_drum, note.program, note.onset, note.velocity, note.pitch)
        )
        ...
        for i, ne in enumerate(note_events):
            ...
            if ne.is_drum:
                ...
                if matched_onset_event_index is not None:
                    ...
                    raise ValueError(
                        f"📕 note2note_event: no matching onset event for {ne}"
                    )
            else:
...
def slice_note_events_and_ties(
...
def slice_multiple_note_events_and_ties_to_bundle(
    note_events: List[NoteEvent],
...
def mix_note_event_lists_bundle(
    ...
    if use_deepcopy is True:
        ...
        tie_note_events_to_mix = deepcopy(
            note_event_lists_bundle_to_mix["tie_note_events"]
        )
    else:
    ...
    if start_time_to_zero is True:
        for note_events, tie_note_events, start_time in zip(
            note_events_to_mix, tie_note_events_to_mix, start_times
        ):
...
def pitch_shift_note_events(
    note_events: List[NoteEvent], semitone: int, use_deepcopy: bool = False
) -> List[NoteEvent]:
    """
...
def separate_by_subunit_programs_from_note_event_lists_bundle(
    ...
    # Reset start time to zero
    if start_time_to_zero is True and not all(t == 0.0 for t in src_start_times):
        for nes, tnes, start_time in zip(
            src_note_events, src_tie_note_events, src_start_times
        ):
            for ne in nes:
                ne.time -= start_time
                assert ne.time >= 0, f"📕 mix_note_events: negative time {ne.time}"
            for tne in tnes:
                tne.time = None
                tne.activity = None
        src_start_times = [0.0 for i in range(len(src_start_times))]
    ...
    result_start_times = [0.0 for _ in range(num_subunits)]
    ...
    for nes, tnes in zip(src_note_events, src_tie_note_events):
        for ne in nes:
            if ne.is_drum:
                target_indices = [
                    i
                    for i, sp_set in enumerate(subunit_program_sets)
                    if DRUM_PROGRAM in sp_set
                ]
            else:
                target_indices = [
                    i
                    for i, sp_set in enumerate(subunit_program_sets)
                    if ne.program in sp_set
                ]
...
def separate_channel_by_program_group_from_note_event_lists_bundle(
    source_note_event_lists_bundle: NoteEventListsBundle,
    ...
    for b, (nes, tnes) in enumerate(zip(src_note_events, src_tie_note_events)):
        ...
        if sort:
            for ch in range(num_program_groups):
                ...
                result_list_bundle[b]["tie_note_events"][ch].sort(
                    key=lambda n_ev: (n_ev.program, n_ev.pitch)
                )
...
def check_event_len_from_bundle(
    note_events_dic_a: Dict,
    ...
    if fast_check is False or total_note_events_len >= max_len // 3:
        event_len_a = 0
        for ne, tne, start_time in zip(
            note_events_dic_a["note_events"],
            note_events_dic_a["tie_note_events"],
        ...
        event_len_b = 0
        for ne, tne, start_time in zip(
            note_events_dic_b["note_events"],
            note_events_dic_b["tie_note_events"],
