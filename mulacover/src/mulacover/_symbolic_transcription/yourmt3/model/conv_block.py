...
def act(x: torch.Tensor, activation: str) -> torch.Tensor:
    ...
    funcs = {
        "relu": F.relu_,
        "leaky_relu": lambda x: F.leaky_relu_(x, 0.01),
...
class Res2DAVPBlock(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size, avp_kernel_size, activation
    ):
    ...
    def init_weights(self):
        for m in [self.conv1, self.conv2] + (
            [self.shortcut] if self.is_shortcut else []
        ):
            init_layer(m)
...
class PreEncoderBlockHFTT(nn.Module):
    ...
    def __init__(self, margin_pre=15, margin_post=16) -> None:
        ...
        self.conv = nn.Conv2d(
            1, 4, kernel_size=(1, 5), padding="same", padding_mode="zeros"
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.pad(
            x, (self.margin_pre, self.margin_post), value=1e-7
        )  # (B, 1, F, T+margin) or (2,1,128,141)
        x = self.conv(x)  # (B, C, F, T+margin) or (2, 4, 128, 141)
        x = x.unfold(
            dimension=3, size=32, step=1
        )  # (B, c1, T, F, c2) or (2, 4, 128, 110, 32)
        x = rearrange(
            x, "b c1 f t c2 -> b t f (c1 c2)"
        )  # (B, T, F, C) or (2, 110, 128, 128)
...
class PreEncoderBlockRes3BHFTT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.pad(
            x, (self.margin_pre, self.margin_post), value=1e-7
        )  # (2, 1024, 141): B,F,T+margin
        x = rearrange(x, "b f t -> b t f")  # (2, 141, 1024): B,T+margin,F
        x = self.res3b(x)  # (2, 141, 128, 4): B,T+margin,F,C
        x = x.unfold(
            dimension=1, size=32, step=1
        )  # (B, T, F, C1, C2) or (2, 110, 128, 4, 32)
        x = rearrange(
            x, "b t f c1 c2 -> b t f (c1 c2)"
        )  # (B, T, F, C) or (2, 110, 128, 128)
        return self.emb_freq(x)  # (B, T, F, C) or (2, 110, 128, 128)
