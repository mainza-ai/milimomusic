...
export const VideoTopBar: React.FC<VideoTopBarProps> = ({
    ...
    return (
        <header className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 p-4 rounded-2xl bg-white/70 dark:bg-black/40 backdrop-blur-xl border border-black/[0.06] dark:border-white/10 shadow-apple-md">
            {/* Left: Track Picker & Status Badges */}
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-1 min-w-0">
                <div className="relative min-w-[200px] sm:max-w-xs">
                    <select
                        value={selectedSongId || ''}
                        onChange={(e) => onSelectSong(e.target.value)}
                        className="w-full text-xs font-semibold rounded-xl bg-black/[0.04] dark:bg-white/5 border border-black/[0.08] dark:border-white/10 px-3 py-2 text-slate-900 dark:text-slate-100 pr-8 truncate focus:outline-none focus:ring-2 focus:ring-teal-500/40 cursor-pointer"
                    >
                ...
                {activeSong && (
                    <div className="flex items-center gap-2 flex-wrap text-[11px]">
                        ...
                        <button
                            ...
                            className={`px-2.5 py-1 rounded-lg font-bold flex items-center gap-1.5 transition-all active:scale-95 shadow-sm ${
                                isThisSongPlaying
                        ...
                        <span className="font-mono px-2 py-0.5 rounded-md bg-black/5 dark:bg-white/5 text-slate-600 dark:text-slate-400">
            ...
            <div className="flex items-center gap-1 bg-black/[0.03] dark:bg-white/5 p-1 rounded-xl border border-black/[0.06] dark:border-white/10 self-start sm:self-auto">
                <button
                    type="button"
            ...
            {/* Right: Master Production Actions (Always Available) */}
            <div className="flex items-center gap-2 flex-wrap justify-end">
                ...
                {renderedVideoUrl && onDownloadVideo && (
                    <button
                        ...
                        className="px-3.5 py-1.5 bg-black/[0.05] dark:bg-white/10 hover:bg-black/[0.08] dark:hover:bg-white/15 text-slate-800 dark:text-slate-200 font-bold text-xs rounded-xl flex items-center space-x-1.5 transition-all"
